...
const TestimonialSection = () => {
  ...
  // Create a wrapped array for infinite scrolling effect
  const wrappedTestimonials = [...testimonials, ...testimonials.slice(0, 3)];
  ...
  useEffect(() => {
    intervalRef.current = setInterval(() => {
      setCurrentIndex((prevIndex) => {
        // When we reach the original set's end, we don't jump immediately
  ...
  // Handle transition when we reach the end
  useEffect(() => {
    if (currentIndex >= testimonials.length) {
      // Create a slight delay before resetting to create smooth transition
      const timer = setTimeout(() => {
        // Turn off transition temporarily to avoid visual jump
        if (testimonialRef.current) {
          testimonialRef.current.style.transition = 'none';
        }
        setCurrentIndex(0);
        // Force a reflow to ensure the transition property is reset
        setTimeout(() => {
          if (testimonialRef.current) {
            testimonialRef.current.style.transition = 'transform 500ms ease-in-out';
          }
        }, 50);
    ...
    <div className="w-full py-16 bg-gradient-to-br from-blue-50 via-white to-green-50 overflow-hidden">
      <div className="container mx-auto px-4">
        ...
        <div className="relative" onMouseEnter={pauseSlider} onMouseLeave={resumeSlider}>
          ...
          >
            {/* Use the wrapped testimonials for infinite scroll effect */}
            {wrappedTestimonials.map((testimonial, index) => (
              <div 
                key={`${testimonial.id}-${index}`}
                className="w-full md:w-1/2 xl:w-1/3 flex-shrink-0 p-4"
              >
                <div className="h-full transform transition-all duration-300 hover:scale-105 hover:rotate-1 perspective-1000">
        ...
        <div className="flex justify-center mt-6 gap-2">
          {Array(testimonials.length).fill(0).map((_, index) => (
            <button
              key={index}
              onClick={() => {
                pauseSlider();
              ...
              className={`w-2.5 h-2.5 rounded-full transition-colors ${
                currentIndex % testimonials.length === index ? 'bg-blue-600' : 'bg-blue-200'
              }`}
