...
<div className={`w-full flex ${step.alignment === "left" ? "justify-start" : "justify-end"}`}>
      <motion.div
        initial="hidden"
        animate={controls}
        variants={variants}
        className={`flex flex-col ${step.alignment === "left" ? "md:flex-row" : "md:flex-row-reverse"} items-center gap-6`}
      >
        {/* Number Circle */}
        ...
        <div className="flex-grow w-full md:w-auto">
          <div className="bg-white/60 backdrop-blur-sm border border-blue-100 rounded-xl p-6 shadow-md hover:shadow-lg transition-shadow duration-300 relative">
            {/* Connecting line to circle (responsive) */}
            <div className={`absolute top-1/2 ${step.alignment === "left" ? "left-0" : "right-0"} w-6 h-1 bg-gradient-to-r from-blue-400 to-green-400 transform -translate-y-1/2 ${step.alignment === "left" ? "-translate-x-full" : "translate-x-full"} hidden md:block`}></div>
